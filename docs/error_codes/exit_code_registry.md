...
13 distinct codes, derived by the commands in §2. `sites` counts literal emission sites; `crates`
...
| `XC-064` | 64 | 6 crates, 11 sites including `fleet-composite` `EXIT_USAGE` | `EX_USAGE` — command line / invocation could not be resolved | a config error (`EX_CONFIG` is 78, `pane-dispatch-fence` `EXIT_CONFIG`). cas: fleet-composite no longer names 64 `EXIT_CONFIG` | correct the invocation |
...
1. `EXIT_CONFIG` name collision — **fixed in cas**: 64 is `fleet-composite` `EXIT_USAGE` (`EX_USAGE`); 78 is `pane-dispatch-fence` `EXIT_CONFIG` (`EX_CONFIG`).
...
| `XC-PT-OUTCOME-EXIT` | `outcome.exit_code()` | `crate-atom-gate` |
