...
| `XC-PT-RESPONSE` | `response.exit_code` | `contabo-reclaim` (`src/main.rs:52`). Forwards a remote worker's response code, so the space genuinely is 0–255 of something we did not run |
