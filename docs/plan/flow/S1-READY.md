...
|R9 has any gate ever FIRED|⚠ **INERT — but WIREABLE for the first time, as of 2026-09-07.** The stated blocker is **CLEARED**: this row said *"named per-layer targets exist for L0/L3/L4/L5 and **NOT L1/L2**"*, and `%7` landed `l1_doctor.rs` and `l2_ecosystem.rs` at `b6b0732`. Measured: **all six layers now have targets** — `l0_install`, `l1_doctor`, `l2_ecosystem`, `l3_step_parity`, `l4_pack` **+** `l4_spawn`, `l5_foundation`. **That is SEVEN targets across SIX layers**, so a runner hardcoding "six" silently skips one. Still INERT: **0** of **12** `gate.yml` jobs name any `l*_` target, 0 in cron. **Wire it, do not write it, and attach ALL SIX AT ONCE** — attaching a subset fires and *reads as coverage*, which is the failure this row exists to name. Attach at `fsu7`'s single Rust entry point, **never seven workflow keys**: Actions is STRICT (n=60, **49 duplicate-key runs started ZERO jobs** — a workflow that fails to parse starts nothing and reports nothing, indistinguishable from a pass). `fsu7` (P0) dispatched to `%19` 2026-09-07; the flip is the orchestrator's, not the implementer's|
...
**S1 IS NOT READY TO BUILD.** At 2026-09-07: **7 PASS · 1 FAIL (R7) · 1 INERT (R9)** — and **R7 and
R9 are the only two rows left**, both in flight (`f3g5` grading with `%7`, `fsu7` with `%19`).

**DO NOT CITE THAT TALLY. DERIVE IT** — count the ✅/❌/⚠ glyphs in the rows above. This line has now
gone stale **four** times inside one session, most recently while the sentence beneath it was being
written, and every stale value was replaced by a fresh integer that went stale in turn. The
correction is not a better number; it is **no number**, which is the same discipline `AGENTS.md`
applies to the package count.
>
