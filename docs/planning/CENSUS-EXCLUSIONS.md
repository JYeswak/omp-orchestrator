...
4. **Assembly is not a second population — but `docs/PLAN.md` is not *purely* assembled.** An id counted in both `docs/PLAN.md` and a section is counted twice, so a census must pick one corpus rather than scale a combined count. **The corpus is: `docs/PLAN.md:1-199` (the constitution head) PLUS the thirteen `docs/plan/[0-9][0-9]-*.md` sections — never PLAN.md's *body* summed with the sections.** The head is not derived from any section and `PLAN.md:6-8` says so verbatim: *"Assembled from `docs/plan/`. Section files remain the source of truth for the body; this header is the constitution's review metadata and companion boundary."* Measured 2026-09-05 by `AtlasArcPass2`: **125 of PLAN.md's 6,259 nonblank lines appear in no section, 103 of them below `:200`.** The concrete cost of getting this wrong: `WP-001`..`WP-009` and the only constitution dependency graph live at `PLAN.md:87-95` and occur in **zero** section files, so a census that picks the sections alone reports the `WS-*`/workstream kind as ABSENT. That happened — to this rule's first obedient reader, in the R2 census (`docs/planning/R2-ID-CENSUS.md`), bead `omp-orchestrator-census-rule4-hides-wp-register-1px7`. `INV-2026` behaves the *opposite* way and is genuinely duplicated (`PLAN.md:40` and `02-surface-census.md`), so the two families need different treatment. **Line-count correction:** the earlier `8,209 vs 10,019` pair compared PLAN.md against `ls -1 docs/plan/*.md` = **19 files**, six of which are not sections (`CONVERGENCE.md`, `CURRENT-REALITY.md`, `R24-SOTA-CHARTER.md`, `ROUNDS.md`, `round24-L1-dag.md`, `skill-ledger-20260902.md`). The like-for-like pair is **8,209 (`wc -l docs/PLAN.md`) vs 7,969 (`cat docs/plan/[0-9][0-9]-*.md | wc -l`)** — a 240-line difference, not 1,810. `HD-9999` is a confirmed duplicate pair (`12-journey.md:471` and `PLAN.md:7577`).
