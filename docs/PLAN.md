...
This plan adopts RFC 2119 and the repository's binding MUST/SHOULD/NEVER rules in `AGENTS.md`.
A conflict between this document, a registry, and implementation stops the affected work pending an ADR.

## Evidence labels

Use the labels established by §6: FACT, DESIGN, HYPOTHESIS, TARGET, NEGATIVE EVIDENCE, and UNKNOWN.
Historical or reported observations remain labelled and do not become current measurements without their
producing command and source revision.
...
|---|---|---|
| `INV-2026` | Retained 2026-08-31 inventory snapshot provenance marker; the full date suffix is descriptive, not a second ID. | §3.2 and §3.3 |

## Non-goals

This pass does not rewrite the plan, remeasure volatile inventory, create beads, freeze registries, or claim
runtime or external validation. Existing product and lifecycle boundaries remain in §§1.2.3, 8, 9, and 12.

## Invariant registry

The `INV-2026` entry above is the sole definition of the retained snapshot invariant. Existing section-level
invariants remain authoritative; this control block does not duplicate them.

## Crate graph / layering

The measured crate graph and layering discussion remain in §3.4 and §3.10; current package and edge figures
resolve through the named live authority there rather than this S2 repair.

## Determinism / test families

Determinism classes, mutation and oracle families, and their evidence rules remain in §6 and §7; this gate
records no new test result.

## Threat model / security / safety

Threats, fail-closed boundaries, and safety limits remain owned by §8 and `docs/plan/06-gates.md`; this
mechanical repair adds no security or runtime claim.

## Do not optimize yet

No optimization is introduced by this repair; performance and SLO decisions remain in §1.4 and later milestone
gates.

## Leapfrog bets

No new bet is asserted here; candidates remain in the existing idea and viability material until a named gate
adopts one.
...
The authoritative verbatim gate lines live in `docs/planning/ROUND_LOG.md`; this table keeps the assembled
plan addressable without restating the evidence.

| Round | Reviewer | Verdict | Diff | Validate |
|---:|---|---|---|---:|
...
theoretical: `pi_agent_rust/src/doctor.rs:check_tool` gates `check_tool` on `output.status.success()` and
...
- **Pre-extraction hash-anchored summary (historical):** 981 row records, 982 nodes, and 1,803 edges. Counts: cli_commands=39, type_roots=57, declarations=14, rpc_handlers=42, slash_commands=799, omp_methods=3, workspace_crates=26; expected_slash_commands=136, so the scanner remains UNKNOWN with exit 2. — HISTORICAL as of 2026-09-02.
...
- **Historical round-10 edge relations (non-authoritative):** provides 157 · map-to-none 25 · path-depends-on 18 · consumes 7. All 7 consumes edges originated from omp-inventory-map; 25 of 26 crates consumed zero OMP surface. — HISTORICAL as of 2026-09-02.
...
> demanded this rebuild as its BLOCKER 1"* and quoted it. The quote is real (`g-adversarial.md:BLOCKER`,
...
|---|---|---|
| an **automated** mutation test | `undrained-pipe-lint/tests/specimens.rs:heading_an_automated_mutation_test_undrained_pipe_lint` `fn mutation_removing_stderr_pipe_retires_violation()` | strongest — the suite runs it |
| a **documented manual** procedure | `no-shell-gate/tests/gate.rs:heading_a_documented_manual_procedure_no_shell_gate` names the three tests that must go RED under mutation: *"A green mutation run would mean the legs are not attributable to the pattern and prove nothing"* | real, but a human must run it |
| a **mutation affordance** | `no-shell-gate/tests/wired_lanes.rs:STRIP_TEST_CODE` — *"The test-code stripping switch is deliberately named so its mutation is attributable"* over `const STRIP_TEST_CODE: bool = true;` | weakest — an invitation, not a leg |
...
|---|---|---|---|
| completion | AgentEndEvent.willContinue + SessionStopEvent (extensibility/shared-events.d.ts:RpcSessionEventFrame), carried by RpcSessionEventFrame (modes/rpc/rpc-types.d.ts:RpcSessionEventFrame) | **WIRE-PROVEN** — raw agent_end frame captured with isTerminal:true | adopt the existing event channel; the remaining gap is wiring it into the supervisor, not inventing a completion protocol |
| receipts | IrcDeliveryReceipt + AsyncJobDeliverySink (tools/hub/types.d.ts:AsyncJobDeliverySink) | **DECLARED ONLY** — no wire path measured | cp-z42vu and the local transport-receipt gap remain; prove reachability before replacing the local receipt contract |
...
> **Upstream type for the idle gap:** `GuestIdleReconcilerCtx` (`dist/types/collab/guest.d.ts:GuestIdleReconcilerCtx`) declares the NewlyIdle/ConfirmedIdle distinction this layer re-derives by hand. DECLARED ONLY: the type exists, no local consumer reads it.
> **Receipts:** `IrcDeliveryReceipt` (`tools/hub/types.d.ts:IrcDeliveryReceipt`) exists upstream. `cp-z42vu` names a defect in what WE consume — a send reporting success while the packet never landed — not an absent upstream type.

**The observe row was re-checked against current source after the Round 21 finding.** The current two-capture implementation compares positive timer or stable-hash motion at tick-monitor/src/lib.rs:Unproven before applying the 75-second floor at :574-577. A changed Working pair is Live; an unchanged short-gap pair is Unproven. The earlier block claiming the floor ran first was a historical description, not current behavior.
...
precedent as `doctor.rs:heading_precedent_as_doctor_rs_924_949_and`, `:949`, and `:950` — all partly right, because each named a **different
...
"appears in NO artifact," and the quote is real at `g-adversarial.md:Underneath`. Underneath the
...
### NO-CLAIM — historical extraction
...
(`tick-monitor/src/lib.rs:PaneState::Unproven`). The refusal shape is `PaneState::Unproven` — *"'I could not
...
**Inputs.** `prev: Option<&Observation>`, `now: &Observation` (`tick-monitor/src/lib.rs:Observation`).
...
**How we know it refused.** The current match at tick-monitor/src/lib.rs:Working is exhaustive with no wildcard arm; the old catch-all that could misclassify Working-to-Idle is absent, and state-wildcard-lint guards that shape. **NO-CLAIM:** a two-capture motion claim cannot tell work from a loop.
### A3. SELECT work
...
(`loop-queue-filter/src/lib.rs:heading_loop_queue_filter_src_lib_rs_134`); a cooldown window.
...
when it was not (`omp-orchestrator/src/lib.rs:heading_when_it_was_not_omp_orchestrator_src`) — a fail-closed arm, not a zero count.
...
(`loop-queue-filter/src/lib.rs:heading_loop_queue_filter_src_lib_rs_399`) and `id.starts_with(&config.epic) && id != config.epic`
...
(`tests/differential.rs:heading_tests_differential_rs_141_145`).
...
(`pane-dispatch-fence/src/main.rs:heading_pane_dispatch_fence_src_main_rs_71`; relative paths rejected).
...
(`tick-monitor/src/lib.rs:NewlyIdle`). One idle capture is one capture; `NewlyIdle` is visible as
...
(`dist/types/extensibility/shared-events.d.ts:heading_dist_types_extensibility_shared_events_d_ts`, *"subscribers must not treat this as a
...
safe_to_dispatch == true` (`fleet-composite/src/main.rs:heading_safe_to_dispatch_true_fleet_composite_src`) — and a wedged pane satisfies both
...
(`tick-monitor/src/lib.rs:heading_tick_monitor_src_lib_rs_352_355`); a packet sent there parks in the composer and never submits.
`Wedged` is checked **first** in `liveness()` (`:497-499`) and is in neither capacity predicate; the
receipt layer names it independently as `WEDGED_UNSUBMITTED` (`receiver-receipt/src/lib.rs:WEDGED_UNSUBMITTED`).
...
(`omp-orchestrator/src/lib.rs:PROJECTED`). **PROJECTED — no measured incident yet**; the expiry sits
...
(`dispatch-claim-fence/src/lib.rs:DispatchIntent`), plus a `DispatchIntent`.
...
(`dist/types/memories/storage.d.ts:WATERMARK`) — an ownership TOKEN plus a WATERMARK, claimed
...
(`dispatch-silence-watch/src/lib.rs:Reassigned`) and has a `Reassigned` arm — *"the original dispatch
...
identifier"* (`dispatch-claim-fence/src/lib.rs:heading_identifier_dispatch_claim_fence_src_lib_rs`).
...
(`ack-stage/src/lib.rs:heading_ack_stage_src_lib_rs_20_26`).
...
`UnprovenTransport` (`receiver-receipt/src/lib.rs:UnprovenTransport`).
...
therefore never part of the receipt proof"* (`receiver-receipt/src/lib.rs:Refuse`). *(b) Refuse to
...
(`pre-delete-citation-check/src/main.rs:PROJECTED`). **PROJECTED — no measured incident yet**; written
...
`IrcDeliveryReceipt` (`dist/types/tools/hub/types.d.ts:AsyncJobDeliverySink`) and `AsyncJobDeliverySink` /
`AsyncJobDeliveryState` (`dist/types/async/job-manager.d.ts:AsyncJobDeliveryState`) — on the IRC-bus and
...
EmptyPaneList | Missing` (`receiver-receipt/src/lib.rs:EmptyPaneList`).
...
(`src/bin/receiver-receipt.rs:heading_src_bin_receiver_receipt_rs_61_64`).
...
(`tick-monitor/src/lib.rs:None`), with `stdout_if_completed()` returning `None` for both
...
(`ack-spine/tests/ack_detector.rs:heading_ack_spine_tests_ack_detector_rs_25`) — a parser rule that holds whatever exit code the tracker
...
**refined** the claim rather than refuting it. `omp-inventory-map/src/types_inventory.rs:heading_refined_the_claim_rather_than_refuting_it`
...
is unbounded because nothing counts it"* (`finding/src/lib.rs:heading_is_unbounded_because_nothing_counts_it_finding`). The second-order failure is
...
dispatch an accounting node (`loop-queue-filter/src/lib.rs:heading_dispatch_an_accounting_node_loop_queue_filter`, `:416`); the closer must refuse the
...
(`pre-delete-citation-check/src/lib.rs:heading_pre_delete_citation_check_src_lib_rs`). The gate scans both surfaces, naming the match via
...
yields NewlyIdle (tick-monitor/src/lib.rs:GuestIdleReconcilerCtx), and the next tick's Idle, Idle yields ConfirmedIdle (:593). The substrate separates the same states with AgentEndEvent.willContinue (:154) and GuestIdleReconcilerCtx; no upstream wire is claimed.
...
NewlyIdle is the current arm at tick-monitor/src/lib.rs:ConfirmedIdle; the old catch-all citation is historical. The actionable filter is fixed: tick-monitor/src/lib.rs:ConfirmedIdle exposes free capacity for ConfirmedIdle or NewlyIdle, and the regression test is omp-orchestrator/src/main.rs:ConfirmedIdle. What remains broken is the SEAM: the producer's field and the consumer's parser agree by convention across a process boundary with no shared type, so a future filter change is invisible again (09 M1). **What would Jeffrey do:** rg -li --type rust -e
...
(`dist/types/extensibility/shared-events.d.ts:heading_dist_types_extensibility_shared_events_d_ts`) separating settle from scheduled
...
(`no-shell-gate/src/lib.rs:heading_no_shell_gate_src_lib_rs_85`, `:56-64`).
...
comment <id> <text>`, which our own doc comment at `dispatch-silence-watch/src/lib.rs:heading_comment_id_text_which_our_own_doc` records
...
mirror HEAD (brief §3.7); `ntm` ships the fuller typed vocabulary (`internal/bv/bv.go:ntm`,
...
4. **Anti-vacuity: an empty scan set is an ERROR, never a pass**, enforced at the choke point so
   callers inherit it (`no-shell-gate/src/lib.rs:Brief`). Brief §3.3 is this failing on our
...
6. **ADDRESSABLE — one documented command runs it, and `--help` names that command.** Added this
   ...
   topic (`coding_agent_session_search/tests/spec_robot_docs_topics.rs:heading_topic_coding_agent_session_search_tests_spec`); adopt it alongside
   the completions/man drift test (`franken_markdown/tests/completions_drift_test.rs:heading_the_completions_man_drift_test_franken_markdown`). §10
   carries both rows.
...
`AgentEndEvent.willContinue` (`dist/types/extensibility/shared-events.d.ts:AgentEndEvent`, WIRE-PROVEN) and
`SessionStopEventResult.continue` (`shared-events.d.ts:SessionStopEventResult`, the actual continuation knob —
...
**1,323 rows:** — HISTORICAL as of 2026-09-02.
...
duration — 469 rows, one dead pid, 12.3 hours. — HISTORICAL as of 2026-09-02.
...
return **92 integration test files** and **984 test functions**. — current authority: NUMBERS.toml figures.test_files / NUMBERS.toml figures.test_functions.
...
**Shape here.** MEASURED: `gate.rs:clean_list_passes`, `clean_list_passes`, pins Rust sources, manifests, markdown, `notes.sh.txt` (FINAL extension only, so the matcher cannot key on a substring), and a dotfile whose stem is not an extension. Those last two are the cases an over-strict extension matcher gets wrong — the boundary, not the obvious pass.

**The measured gap.** `path-literal-guard` has **known_good = 0**. It scans every `.rs` under `crates/*/src` for home-path literals and asserts zero hits (`tests/repo_wide.rs:heading_the_measured_gap_path_literal_guard_has`); it has anti-vacuity (20-26) and a known-bad, but nothing pins what a *legitimate* path expression looks like. It can become arbitrarily strict with no test going RED, and it is repo-wide, so the blast radius is every crate. **`path-literal-guard` is the highest-risk gate in the set** on this measurement, despite being the smallest.
...
**(1) Anti-vacuity guards live INSIDE the metamorphic relation.** `src/runtime/scheduler/metamorphic_tests.rs:CORRECTNESS` asserts, before checking its relation: *"MR4 VIOLATION: zero cancel dispatches across {} injected cancel tasks — streak-bound assertion would be vacuous"*; `:517-522` repeats it for MR5 labelled "ABSOLUTE-CORRECTNESS ANCHOR", and `:661-662` for MR7 — *"timed-lane dispatched {} of {} injected tasks — EDF ordering check would be vacuous on the missing tasks."* The relation does not merely hold; the test first proves the workload was actually exercised. **This is the leg that would have caught `pane-truth`** (§2.9): a fixture-format drift means zero live payloads were classified, and a guard asserting "at least N inputs reached the classifier" goes RED where a green relation says nothing.

**(2) Vacuity can be a typed state rather than a failure.** `src/messaging/jetstream.rs:VACUOUS_INVARIANT_SET` serialises `waiter_fairness_mode: "vacuous_zero_wait_refusal"` into production telemetry, and `src/messaging/jetstream_flow_control_audit.rs:VACUOUS_INVARIANT_SET` explains it: *"for the current controller, fairness is vacuous because hidden waiters are impossible (`max_waiters = 0`)."* The system does not claim fairness and does not fail; it **names its own vacuity as a first-class value**, and `scripts/run_jetstream_publish_backpressure_smoke.sh:181-186` gates on that exact string. This is strictly better than the binary FAIL this subsection specified, for the case where vacuity is *legitimate and permanent*: a trivially-satisfied invariant carried forward under a name a later reader cannot mistake for evidence. Our `VACUOUS_INVARIANT_SET` is the right response to *accidental* vacuity; this is the right response to *structural* vacuity, and we had no vocabulary for the second.

**(3) A positive control is justified as anti-vacuity, fusing §2.2 and §2.4.** `tests/atp_rq_observability_metrics.rs:Positive`: *"Positive control: the manifest DOES carry its content-descriptor fields, so the negative assertions above are meaningful (not vacuous on an empty blob)."* The known-good leg exists *because* the attack leg would otherwise be vacuous — one mechanism, not two. Same shape at `src/stream/buffered.rs:Positive` ("two empty or two all-identical sequences would compare equal and prove nothing"), `src/runtime/builder.rs:Positive` ("an empty fingerprint would make the equality below vacuous"), and `tests/three_lane_tests.rs:Positive` ("a pass here means the worker never touched the shard — vacuous").

**(4) Anti-vacuity is pushed into the type system and the API return value.** `src/trace/tla_export.rs:AllOk` declares `pub type EntityKey = (u32, u32)` precisely so slot-reuse aliasing cannot make invariants "pass *vacuously* because one entity silently overwrote the other in the map." And `src/combinator/map_reduce.rs:AllOk` has `all_succeeded()` return **false** on empty input "even though the aggregate decision is `AllOk` (vacuously true)" — refusing to let a caller read vacuous truth as success, with the test at `:728-729`. Measured yield of the discipline: `CHANGELOG.md:AllOk` records six HTTP/1.1 RFC 9112 tests "that previously **passed vacuously** when codec validation was missing," and `audit_index.jsonl:AllOk` a metamorphic MR2 that "**was vacuous** because it toggled an unrelated testing Cx instead of the inspected runtime state" — our exact defect, found by audit, in his repo. — HISTORICAL as of 2026-09-02.
...
**Why not weaker.** The weak form is a hand-listed expectation set, and `no-shell-gate/tests/wired_lanes.rs:empty_scan_sets_are_errors_not_passes` (snapshot 995a147) names the prior art it avoids: control-plane's `check.sh` hand-lists `EXPECTED_GATES` while the verdict claims completeness, so "the list drifts and the suite reports vacuously green while most lanes are unexamined." Here the set is derived from disk (`derive_lanes`, :50-86; `workspace_crate_names`, :561-577 — line numbers at snapshot 995a147, constructs verified by name) so a new crate is in scope the moment it exists, and an empty or unreadable derivation is an ERROR (`empty_scan_sets_are_errors_not_passes`, 473-483).
...
**A row that must be restated as BLOCKED, not pending.** `ALLOWED_COLLISIONS` deliberately omits `Observation` (`types_inventory.rs:ALLOWED_COLLISIONS`) so the gate REFUSES that collision "until the convergence lands," and the convergence would be reached by adopting the shared vocabulary `omp-types` re-exports. MEASURED, and it changes the row's meaning: `ObligationLedger` occurs **zero** times in `omp-types`, and `AckKind`/`DeliveryClass` appear only in a doc comment naming them as blocked. They sit behind `#[cfg(feature = "messaging-fabric")]`, which transitively requires `test-internals`, which upstream issue #46 correctly removed from default features. So the half of the vocabulary that would collapse our 17 ack/receipt types across 3 incompatible dialects is **unreachable at our pinned rev** — blocked at an upstream feature boundary, not merely unadopted and not pending our work.
...
crates returns headers in exactly three of them — `no-shell-gate/src/lib.rs:heading_crates_returns_headers_in_exactly_three_of`
("WHAT STILL PASSES — do not read this gate as more than it is"),
`omp-inventory-map/src/types_inventory.rs:SEMANTICS` ("this proves SHAPE, not SEMANTICS"), and
`path-literal-guard/src/lib.rs:UNMEASURED` ("WHAT STILL PASSES"). Property 6 is UNMEASURED for seven
...
**RETRACTED: "the matrix has 16 rows and double-counts."** It does not. A stopping reader — one — HISTORICAL as of 2026-09-02.
...
5. It carries anti-vacuity at every level it has — unit, end-to-end, CLI — with gate-error distinguished from violation-found by exit code (the `no-shell-gate` 2-versus-1 convention). Non-emptiness is insufficient: the gate asserts a **non-zero expected floor** on its scan set (e.g. "at least 26 crates," "at least one tracked file per declared lane"), because §1's silently-empty tool proves a scan can collapse to a plausible-looking small number without erroring. A floor turns "I scanned something" into a falsifiable claim. — HISTORICAL as of 2026-09-02.
...
9. It is reachable from `cargo test` — not only from a `main.rs` subcommand a human must remember. MEASURED precedent: `tick-monitor`'s binary `--selftest` has 41 legs and `cargo test` reaches none of them, so its invariants were moved to where the suite executes them (`tests/monitor.rs:MEASURED`).
...
12. It never reads an exit status as evidence of a successful probe. Presence and health are separate claims requiring separate evidence — a path resolution establishes presence, a content check establishes health, and a non-zero status establishes neither absence nor ill-health on its own. Precedent, its tests, and its gap: `pi_agent_rust/src/doctor.rs:check_tool_reports_invocation_failure_for_broken_executable` (the two-signal fallback arm), pinned by `:13948` `check_tool_falls_back_when_probe_args_are_unsupported` and `:13964` `check_tool_reports_invocation_failure_for_broken_executable` (§1).
...
**409 tests as a proxy metric.** If the count becomes the target, the count will rise and the leg table will not. The only numbers worth reporting are the leg table and the §3.1 matrix, and the only honest headlines today are **2 of 8** on four legs and **0 of 8** on six properties. — HISTORICAL as of 2026-09-02.
...
`dist/types/extensibility/shared-events.d.ts:RpcSessionEventFrame`, carried on `RpcSessionEventFrame`
(`modes/rpc/rpc-types.d.ts:heading_modes_rpc_rpc_types_d_ts_589`). The objection survives only in the weaker form *"we do not ride
...
## Appendices B–K — MOVED to `docs/inventories/`

The ten surface-coverage appendices that used to run from here to the end of this section now live
one document per wave under `docs/inventories/`, with a roll-up at
[`omp_surface_coverage_index.md`](../inventories/omp_surface_coverage_index.md). Nothing was
deleted: **47,220 bytes** moved out of a **125,048-byte** section, verbatim apart from a one-level
heading demotion and a per-document Purpose / Cross-References / Validation wrapper.

**Where the seam is, and why it is there.** Everything above this line is the nine-stage journey
runbook — a procedure, with a Trigger, a Dispatch packet and a Done signal per stage, read in order
by an operator who is about to do something. Everything below it was a classification table: one
row per OMP surface against the eight-clause per-crate contract, read by a maintainer looking one
surface up. The two were welded together because the sweeps happened to be run while the runbook
was being written, not because they share a concern. A runbook that is 62% lookup table is a
runbook nobody finishes reading.

| wave | name | surfaces | document |
|---|---|---|---|
...
| `ipg.8` | EDIT | edit, lsp, commit, compress, cleanse, markit | [`omp_surface_coverage_ipg8.md`](../inventories/omp_surface_coverage_ipg8.md) |
| `ipg.9` | SECURITY | secrets, security, extensibility, config | [`omp_surface_coverage_ipg9.md`](../inventories/omp_surface_coverage_ipg9.md) |
...
second, different split dressed up as the same one.
