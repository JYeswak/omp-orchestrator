...
> **9 measured claims in this document were refuted while it was being written**, and every one
> was caught by an agent re-deriving rather than reading, or by following a `NO-CLAIM` into the
> source. They are kept as labelled retractions rather than quietly corrected, because the failures
> are more instructive than the corrected values. **This count is generated from the table in
> `00-brief.md` §7 at assembly time** — it was previously written twice and disagreed with itself,
> which is how a self-correction section came to be unable to count its own corrections.

> The plan also carries **12 open questions** and **5 kill criteria** in §8, all unanswered,
> registered so they are at least assignable.
