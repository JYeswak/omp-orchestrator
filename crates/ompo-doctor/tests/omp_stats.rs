//! Integration legs for `ompo stats` — the verb that drives ONE bounded OMP `--mode=rpc`
//! session, issues OMP's native `get_session_stats`, and reports OMP's own answer.
//!
//! Every leg here is HOST-INDEPENDENT by construction. `ompo stats` reaches a real `omp`
...
//!
//! ```text
//! 0  OMP answered and the session-stats payload is present
//! 1  OMP answered and REFUSED, or answered successfully with no stats payload
...
#[test]
fn the_stats_verb_is_reachable_and_never_reports_an_unknown_verb() {
    let out = ompo().arg("stats").output().expect("ompo runs");
    let code = out.status.code();
    assert!(
        matches!(code, Some(0) | Some(1) | Some(4)),
        "`ompo stats` must answer from its own exit dictionary (0, 1 or 4) and never 2, which \
...
#[test]
fn the_stats_verb_appears_in_capabilities_and_the_two_halves_agree() {
    let out = ompo()
    ...
    assert!(
        declared.contains(&"stats"),
        "the dispatched verb `stats` MUST be declared in capabilities; got {declared:?}"
    );
}
...
/// while `OMP_STATS_ABSENT` / `OMP_STATS_TRANSPORT_FAILED` mean go look at the machine. A
...
#[test]
fn an_unknown_flag_on_stats_is_an_invocation_error_not_a_transport_failure() {
    let out = ompo()
        .args(["stats", "--zzz-not-a-flag"])
        .output()
        .expect("ompo runs");
    assert_eq!(
        out.status.code(),
    ...
    let stderr = String::from_utf8_lossy(&out.stderr);
    for forbidden in ["OMP_STATS_ABSENT", "OMP_STATS_TRANSPORT_FAILED"] {
        assert!(
...
/// act on. Which specific code appears is host-dependent — `OMP_STATS_ABSENT` on a machine
/// with no `omp`, `OMP_STATS_REFUSED` or `OMP_STATS_NO_PAYLOAD` on one where OMP answered —
/// so only the FAMILY is asserted, which is the strongest host-independent claim available.
#[test]
fn every_non_zero_stats_path_names_a_typed_reason_code() {
    let out = ompo().arg("stats").output().expect("ompo runs");
    ...
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        stderr.contains("OMP_STATS_"),
        "a non-zero `ompo stats` (exit {:?}) must emit a typed OMP_STATS_* reason, never a \
...
/// carry its documented keys AND the adopted method must be OMP's native `get_session_stats`
/// (not a substituted or invented call); on any non-zero exit stdout must NOT parse as an
/// object.
#[test]
fn the_json_shape_is_stable_whenever_the_stats_verb_succeeds() {
    let out = ompo().args(["stats", "--json"]).output().expect("ompo runs");
    let stdout = String::from_utf8_lossy(&out.stdout);
    ...
    if out.status.code() == Some(0) {
        ...
        assert_eq!(
            value["data"]["adopted_method"], "get_session_stats",
            "the adopted method must be OMP's native `get_session_stats`, not a substitute; \
             got {value:?}"
        );
        return;
    }
    ...
    let looks_like_an_envelope = parsed.as_ref().is_some_and(serde_json::Value::is_object);
    assert!(
...
/// dictionary.
#[test]
fn two_identical_stats_runs_agree_on_their_exit_code() {
    let first = ompo().args(["stats", "--json"]).output().expect("ompo runs");
    let second = ompo().args(["stats", "--json"]).output().expect("ompo runs");
    assert_eq!(
        first.status.code(),
        second.status.code(),
        "two identical `ompo stats --json` runs must agree on their exit code; got {:?} then \
         {:?} (stderr {:?} / {:?})",
        first.status.code(),
        second.status.code(),
