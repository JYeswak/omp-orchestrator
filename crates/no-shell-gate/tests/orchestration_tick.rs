...
fn validate_receipt(receipt: &Value) -> Result<(), Vec<String>> {
    ...
    // OC-L1: every observed free pane is named exactly once, and no non-free
    // pane is smuggled into the union.
    ...
    // OC-L2: the receipt may only name a bead after the tracker claim.
    ...
    // OC-L4: measured claims carry the command that produced them.
    ...
    // OC-L6: destructive actions name a prior receipt row as evidence.
...
fn read_ledger(path: &Path) -> Result<Vec<Value>, LedgerError> {
    ...
    if bytes.iter().all(u8::is_ascii_whitespace) {
        return Err(LedgerError::NothingToCheck(
            "NOTHING_TO_CHECK: orchestration ledger has zero receipt rows".to_owned(),
        ));
    }
    ...
    for (index, line) in bytes.split(|byte| *byte == b'\n').enumerate() {
        ...
        let row = serde_json::from_slice(line).map_err(|error| {
            LedgerError::Invalid(format!(
                "invalid orchestration receipt row {}: {error}",
                index + 1
            ))
        })?;
        rows.push(row);
