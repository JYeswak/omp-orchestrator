    ...
    for line in text.lines() {
            ...
            .and_then(|s| s.strip_suffix(']'))
        {
            ...
            if l.starts_with("path") {
                cur.path = val(l).unwrap_or_default();
            } else if l.starts_with("format") {
                cur.format = val(l).unwrap_or_default();
...
fn the_registry_is_not_empty_and_every_row_is_complete() {
    ...
    assert!(
        d.len() >= 5,
        "registry declares {} artifacts; it described 6 when written",
        d.len()
    );
    for a in &d {
...
fn every_declared_artifact_on_disk_carries_its_required_fields() {
    ...
    for a in declarations() {
        ...
        if a.path.contains('<') || a.path.starts_with('~') {
            continue;
        }
...
fn every_severity_row_names_its_grader() {
    ...
    assert!(
        checked > 0,
        "ANTI-VACUITY: no severity rows found — the scan is broken or the schema was never adopted"
    );
    assert!(
...
fn the_newest_round_of_grade_evidence_carries_severity() {
    ...
    let Ok(entries) = fs::read_dir(dir) else {
        eprintln!(
            "SKIP: /tmp/grade absent — evidence does not survive a reboot, which is \
    ...
    for p in &current {
        ...
        let tagged = ["BLOCKER", "MAJOR", "MINOR", "SEVERITY"]
            .iter()
            .any(|k| t.contains(k));
        if !tagged {
    ...
    assert!(
        ...
        "round {newest} evidence carries no severity tag, so it cannot be compared with any \
         ...
         premise, and both were filed this session.",
        untagged.join("\n  ")
    );
}
