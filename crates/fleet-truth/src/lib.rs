...
impl Rule {
    pub const ALL: &'static [Rule] = &[
        Rule::IdentityUnknownRanksHigh,
        Rule::GroundTruthNotClassifier,
        Rule::ZeroCommitsInspectFirst,
    ];

    pub fn as_str(self) -> &'static str {
        match self {
            Rule::IdentityUnknownRanksHigh => "identity_unknown_ranks_high",
            Rule::GroundTruthNotClassifier => "ground_truth_not_classifier",
            Rule::ZeroCommitsInspectFirst => "zero_commits_inspect_first",
        }
    }
...
impl Rules {
    pub fn disable(&mut self, name: &str) -> bool {
        ...
        match rule {
            Rule::IdentityUnknownRanksHigh => self.identity_unknown_ranks_high = false,
            Rule::GroundTruthNotClassifier => self.ground_truth_not_classifier = false,
            Rule::ZeroCommitsInspectFirst => self.zero_commits_inspect_first = false,
        }
        true
    }
...
pub fn truth_row(s: &Sensors, rules: &Rules) -> TruthRow {
    if s.vstate != "OK" {
...
mod tests {
    ...
    fn identity_unknown_ranks_999() {
        ...
        let row = truth_row(&s, &Rules::default());
        assert_eq!(
            row.score, 999,
