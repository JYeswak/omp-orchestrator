...
fn all_fresh_fixture_maps_every_row_and_gates_ok() {
    ...
    assert_eq!(verdicts.len(), 2, "both rows map, count unpinned by contract");
    ...
    let wc = verdicts.iter().find(|v| v.source == "work_coordination").expect("key preserved");
...
fn unavailable_row_is_refusing_and_names_source() {
    ...
    let dark = verdicts.iter().find(|v| v.source == "dark_source").expect("dark row mapped");
    ...
    match &error {
        NtmSourceError::StaleSources { sources } => {
            assert!(sources.iter().any(|s| s.contains("dark_source")), "source named: {sources:?}");
        }
        other => panic!("expected StaleSources, got {other}"),
    }
    let text = error.to_string();
    assert!(text.starts_with("NTM_SOURCE_STALE") && text.contains("dark_source"), "diagnostic: {text}");
...
fn stale_row_is_silent_and_vetoes() {
    ...
    assert!(matches!(error, NtmSourceError::StaleSources { .. }), "got {error}");
...
fn missing_fields_are_distinct_restrictive_errors() {
    for (field, row) in [
        ("available", json!({"fresh": true, "reason_code": "health:ok", "age_ms": 1})),
        ("fresh", json!({"available": true, "reason_code": "health:ok", "age_ms": 1})),
        ("reason_code", json!({"available": true, "fresh": true, "age_ms": 1})),
        ("age_ms", json!({"available": true, "fresh": true, "reason_code": "health:ok"})),
    ] {
...
fn wrong_types_are_distinct_restrictive_errors() {
    for (field, row) in [
        ("available", json!({"available": "yes", "fresh": true, "reason_code": "r", "age_ms": 1})),
        ("fresh", json!({"available": true, "fresh": 1, "reason_code": "r", "age_ms": 1})),
        ("reason_code", json!({"available": true, "fresh": true, "reason_code": 42, "age_ms": 1})),
        ("age_ms", json!({"available": true, "fresh": true, "reason_code": "r", "age_ms": -5})),
        ("age_ms", json!({"available": true, "fresh": true, "reason_code": "r", "age_ms": 1.5})),
        ("age_ms", json!({"available": true, "fresh": true, "reason_code": "r", "age_ms": "old"})),
    ] {
        let snapshot = snapshot_with(json!({ "typed_source": row }));
        let error = parse_ntm_sources(&snapshot).expect_err("wrong type must refuse");
        match &error {
            NtmSourceError::WrongType { source, field: got, .. } => {
...
fn empty_reason_is_its_own_error_not_missing() {
    ...
    assert!(matches!(error, NtmSourceError::EmptyReason { .. }), "got {error}");
...
fn exit_codes_are_pinned_per_class() {
    ...
    assert_eq!(ntm_source_exit_code(&NtmSourceError::EmptySources), ExitCode::from(2));
    assert_eq!(
        ntm_source_exit_code(&NtmSourceError::SnapshotUnavailable { detail: "x".into() }),
        ExitCode::from(3)
    );
    assert_eq!(
        ntm_source_exit_code(&NtmSourceError::StaleSources { sources: vec!["a".into()] }),
        ExitCode::from(1)
    );
    assert_eq!(
        ntm_source_exit_code(&NtmSourceError::MissingField { source: "s".into(), field: "available" }),
        ExitCode::from(1)
    );
...
fn empty_sources_object_is_typed_empty_scan() {
    ...
    assert_eq!(error.to_string(), "NTM_SOURCE_EMPTY_SCAN — empty is ERROR, never a pass");
...
fn missing_envelope_is_unmeasured_never_absence() {
    ...
    assert!(matches!(error, NtmSourceError::MalformedSources { .. }), "got {error}");
...
fn live_daemon_smoke_is_labelled_mutable() {
    ...
    // pins the mutable population.
    match read_live_snapshot("ntm") {
        Ok(snapshot) => {
            let verdicts = parse_ntm_sources(&snapshot).expect("live shape must parse");
            assert!(!verdicts.is_empty(), "live population must be nonempty to smoke");
            for mapped in &verdicts {
            ...
            println!(
                ...
                verdicts.iter().map(|v| v.source.clone()).collect::<Vec<_>>().join(","),
                if aggregate.is_ok() { "fresh" } else { "NOT_FRESH" },
            );
        }
        Err(error) => {
            assert!(
