...
/// How long we WAIT for ntm before signalling its process group. Generous: ntm spawn
/// does real work (session creation, pane layout) and a false timeout is worse than a
/// slow one -- it would kill a healthy session mid-create.
const NTM_SPAWN_DEADLINE: std::time::Duration = std::time::Duration::from_secs(120);
...
fn spawn(args: &[String]) -> Result<ExitCode, String> {
    ...
    // A bare `.status()` here had NO DEADLINE: a hung ntm blocked the operator forever,
    // with no typed outcome and no way to tell "ntm is thinking" from "ntm is wedged".
    // `bounded_status` is the passthrough sibling of `bounded_output` -- it INHERITS
    // stdio, so ntm's interactive output still reaches the terminal (capturing it here
    // would break the whole point of a passthrough spawn), while still making the child
    // its own process-group leader and signalling the GROUP on the deadline so
    // grandchildren cannot survive as orphans at ppid=1.
    match subprocess_contract::bounded_status(&mut command, NTM_SPAWN_DEADLINE) {
        subprocess_contract::BoundedOutcome::Completed(out) => Ok(if out.status.success() {
            ExitCode::SUCCESS
        } else {
            ExitCode::from(1)
        }),
        // RESTRICTIVE. A killed ntm is not a failed ntm, and the operator needs to know
        // which one happened -- the remedies differ.
        subprocess_contract::BoundedOutcome::TimedOut => Err(format!(
            "SCRATCH_HOME_NTM_TIMEOUT ntm did not return within {}s; process group \
             signalled. This is NOT an ntm failure verdict - no exit status was observed.",
            NTM_SPAWN_DEADLINE.as_secs()
        )),
        subprocess_contract::BoundedOutcome::Unspawned(error) => {
            Err(format!("ntm spawn failed: {error}"))
        }
    }
}
