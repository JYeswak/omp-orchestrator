...
fn planted_real_ledger_names_close_evidence_red_through_binary() {
    ...
    let (rc, out, _err) = run(&["--ledger", fx.to_str().unwrap()], &[]);
    assert_eq!(rc, 0);
    assert!(
...
fn empty_comparison_is_not_a_pass() {
    ...
    let (rc, _out, err) = run(&["--ledger", missing.to_str().unwrap()], &[]);
    assert_eq!(rc, 3, "missing comparison input must fail closed");
    assert!(
        err.contains("ERROR ledger_missing"),
        "rule anti_vacuity: missing ledger must emit a typed error, got {err:?}"
    );
    assert!(
        err.contains(missing.to_str().unwrap()),
        "rule anti_vacuity: missing ledger error must name the path, got {err:?}"
    );
    assert!(
        err.contains("reason="),
        "rule anti_vacuity: missing ledger error must name the reason, got {err:?}"
    );
}