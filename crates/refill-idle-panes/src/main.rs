...
use refill_idle_panes::{
    dispatchable_panes, packet_is_sendable, parse_recommendations, plan, survey, Assignment,
};
use std::io::Write;
...
fn run(apply: bool) -> ExitCode {
    ...
    if panes.is_empty() {
        // NOT AN ERROR. A busy fleet is the healthy state; shouting about it makes this
        // noisy enough to be ignored, and an ignored tool protects nothing.
        println!(
            "refill: no idle pane both surfaces agree on — nothing to do{}",
            if s.unreadable { " (a probe was UNREADABLE — refusing all)" } else { "" }
        );
