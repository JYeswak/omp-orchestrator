...
pub use receiver_receipt::ReceiptVerdict;
...
///
/// The receipt is the canonical [`receiver_receipt::ReceiptVerdict`]. The
/// `Observed` wrapper records that the receipt was supplied as delivery
/// evidence, but only `ReceiptConfirmed` is positive delivery proof; a
/// `NoReceipt`, `Dead`, or `Indeterminate` verdict remains non-delivery.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum DeliveryAuthority {
    Observed { receipt: ReceiptVerdict },
...
mod tests {
    ...
    fn receipt_confirmed() -> ReceiptVerdict {
    ...
    fn canonical_non_confirmed_verdicts_are_not_delivery() {
        for receipt in [
            ReceiptVerdict::NoReceipt {
                pane_id: "%1408".to_owned(),
                reason: receiver_receipt::ReceiptReason::IdleUnchanged,
            },
            ReceiptVerdict::Dead {
                pane_id: "%1408".to_owned(),
            },
            ReceiptVerdict::Indeterminate {
                pane_id: "%1408".to_owned(),
                reason: receiver_receipt::ReceiptReason::MissingPostObservation,
            },
        ] {
